...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, company, projectType, tradeType, projectState, budget, message, honeypot } = body;
    ...
    // Basic validation
    if (!name || !email || !phone || !projectType || !tradeType || !projectState || !message) {
    ...
    // --------------------------------------------------------
    // Resend integration — replace RESEND_API_KEY in .env.local
    // --------------------------------------------------------
    ...
    if (RESEND_API_KEY && RESEND_API_KEY !== "YOUR_RESEND_API_KEY") {
      ...
      await resend.emails.send({
        ...
        to: [process.env.CONTACT_EMAIL ?? "EMAIL_PLACEHOLDER"],
          ...
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #F6F8FB; padding: 24px; border-radius: 0 0 8px 8px; border: 1px solid #E6EAF0;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">Phone</td><td style="color: #1B1F24;">${phone}</td></tr>
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">Company</td><td style="color: #1B1F24;">${company || "N/A"}</td></tr>
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">Project Type</td><td style="color: #1B1F24;">${projectType}</td></tr>
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">Trade</td><td style="color: #1B1F24;">${tradeType}</td></tr>
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">State</td><td style="color: #1B1F24;">${projectState}</td></tr>
                <tr><td style="padding: 8px 0; font-weight: bold; color: #374151;">Budget</td><td style="color: #1B1F24;">${budget || "Not specified"}</td></tr>
              </table>
              <div style="margin-top: 16px; padding: 16px; background: white; border-radius: 8px; border: 1px solid #E6EAF0;">
                <p style="font-weight: bold; color: #374151; margin: 0 0 8px;">Project Description</p>
          ...
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="padding: 24px; border: 1px solid #E6EAF0; border-radius: 0 0 8px 8px;">
              ...
              <div style="margin: 20px 0; padding: 16px; background: #EBF2FF; border-radius: 8px; border-left: 4px solid #1565D8;">
                ...
                <p style="margin: 8px 0 0; color: #374151; font-size: 14px;">If you have plans ready, please email them to EMAIL_PLACEHOLDER to speed up the process.</p>
              </div>
              <p style="color: #6B7280; font-size: 14px;">Questions? Call us at PHONE_PLACEHOLDER or reply to this email.</p>
            </div>
          </div>
        `,
      });
